...
test_that("reading a 1D histogram from file", {
	...
	expect_equal( mean(h1$entries)     , 19.23077, tolerance = 1.E-6, scale = mean(h1$entries) )
	expect_equal( mean(h1$weightedMean), 25.01852, tolerance = 1.E-6, scale = mean(h1$weightedMean) )
	expect_equal( mean(h1$error)       , 4.037106, tolerance = 1.E-6, scale = mean(h1$error)  )
...
test_that("getAnnotation", {
	...
	expect_equal( a10$values[[3]], "983" )
	expect_equal( a10$values[[4]], "24.563" )	
})
